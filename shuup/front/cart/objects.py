...
from shuup.front.cart.storage import CartCompatibilityError, get_storage
...
class BaseCart(OrderSource):
    def __init__(self, request, cart_name="cart"):
        super(BaseCart, self).__init__(request.shop)
    ...
    def add_code(self, code):
        modified = super(BaseCart, self).add_code(code)
        self.dirty = bool(self.dirty or modified)
        return modified

    def clear_codes(self):
        modified = super(BaseCart, self).clear_codes()
        self.dirty = bool(self.dirty or modified)
        return modified

    def remove_code(self, code):
        modified = super(BaseCart, self).remove_code(code)
        self.dirty = bool(self.dirty or modified)
        return modified
    ...
    def get_validation_errors(self):
        for error in super(BaseCart, self).get_validation_errors():
            yield error
