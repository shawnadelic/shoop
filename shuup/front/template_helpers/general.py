...
def get_listed_products(context, n_products, ordering=None, filter_dict=None, orderable_only=True):
    ...
    if not filter_dict:
        filter_dict = {}
    products_qs = Product.objects.visible(
        shop=shop,
        customer=customer,
        language=get_language(),
    ).listed(shop=shop, customer=customer).filter(**filter_dict)
    if ordering:
...
def get_all_manufacturers(context):
    request = context["request"]
    products = Product.objects.visible(shop=request.shop, customer=request.customer)
