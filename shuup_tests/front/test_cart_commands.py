...
from shuup.front.cart import commands as cart_commands
from shuup.front.cart import get_cart_command_dispatcher
from shuup.front.cart.command_dispatcher import CartCommandDispatcher
from shuup.front.signals import get_cart_command_handler
from shuup.testing.factories import (
    create_product, get_default_product, get_default_shop,
    get_default_supplier
)
from shuup_tests.front.fixtures import get_request_with_cart
...
def test_variation():
    ...
    cart_commands.handle_add_var(request, cart, parent.id, **{"var_%s" % attr.id: val.id})
    assert cart.get_product_ids_and_quantities()[child.pk] == 1

    with pytest.raises(ValidationError):
        cart_commands.handle_add_var(request, cart, parent.id, **{"var_%s" % attr.id: (val.id + 1)})
...
def test_complex_variation():
    ...
    cart_commands.handle_add_var(request, cart, parent.id, **kwargs)
    assert cart.get_product_ids_and_quantities()[child.pk] == 1
